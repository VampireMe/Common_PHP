...
class UploadFile {
    ...
    public $file_name;//默认是使用原图名称（"date"：使用当前上传的时间字符串作为名称；其他：使用指定的名称）
     ...
     */
    public function __construct($files, $file_name = '', $save_path = './upload/', $allow_types = '') {
        $this->files = $files;
    ...
    public function upload_file() {
        ...
        if($error == 0 && is_uploaded_file($tmp_name)) {
            ...
            $new_name = $this->getNewFileName($this->file_name);
     ...
     * @author gaoqing
     * 2015年7月1日
     * @param string $fileNameFlag 新文件名称生成规则标识（""：使用原文件名；"date"：使用当前上传时间为文件名；其他：使用指定文件名）
     * @return string 新文件名
     */
    private function getNewFileName($fileNameFlag){
        
        $newFileName = "";
        if (isset($fileNameFlag)) {
            switch ($fileNameFlag){
                case "" : $newFileName = $this->files['name']; 
                    break;
                case "date" : $newFileName = date('YmdHis') . '.' . $this->ext;
                    break;
                default: $newFileName = $fileNameFlag . '.' . $this->ext;
                    break;
            }
        }
        return $newFileName;
